...
class MultiDocumentChatAzureOpenAI:
    ...
    def _load_pdf(self,path:str=None):
        if path==None:
            loader = PyPDFDirectoryLoader("hp_docs/")
        else:
            loader = PyPDFDirectoryLoader("hp_docs/")
    ...
    def _init_db(self,data):
        persist_directory = ACTIVE_WORKFLOW["persist_directory"]
        #docsearch = Chroma.from_documents(data, self.embeddings,persist_directory=persist_directory)
        #docsearch = FAISS.from_texts(data, self.embeddings)#persist_directory=persist_directory)
        #docsearch.persist()
