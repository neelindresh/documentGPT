...
class SingleChatAzureOpenAI:
    def __init__(self,pdf_path=None) -> None:
        ...
        if not os.path.exists(persist_directory):
            ...
            docsearch=self._init_db(chunks)
        '''
        data=self._load_pdf_folder(pdf_path)
        chunks=self._preprocessor(data)
        docsearch=self._nostorage_(chunks)
        template=self._prompt_template()
        self._qa(docsearch,template)
        self.chat_history=[]
        
    def _load_pdf_folder(self,pdf_path:str=None):
        if pdf_path!=None:
    ...
    def _nostorage_(self,data):
        if ACTIVE_WORKFLOW["splitOn"]=="text":
